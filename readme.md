...
# Implementation
## The Ada runtime
Now that we know how to export Ada code to C we can start writing our kernel module.\
In the previous example we have used the default Ada runtime however, this runtime is unusable in kernel land, and we need to adapt it.
...
└── src
```
The `adalib` directory contains the Ada runtime library. \
In our case `adainclude` and `src` will contain the same files.
...
Because we will not use any standart library we need to ensure that
`Suppress_Standard_Library` is set to `True` in `system.ads`.

## Building the runtime

To build the runtime we are going use the `gprbuild` tool. So we need to create a `runtime.gpr` file.
```ada
...
In addition we need to create a `runtime.adc` file to configure global options for the runtime.
A solid start is to reuse the file `gnat.adc` by `Luke A. Guest`
```ada
--                              -*- Mode: Ada -*-
--  Filename        : gnat.adc
...
```
Once those file are in place we can write the makefile for the runtiem:
```makefile
...
An helper containing wrapper function for the Ada code:
```c
// src/helper.c
//src/helper.c
...
```
The content of the file is pretty standard we need to specify the switch for gnatgcc and the runtime we are using for building.\
Because we compiled the runtime with the pragma normalize scalar we also need to compile the source code with this pragma so we just add this line in `ada.adc`
```ada
pragma Normalize_Scalars;
```
# Linking with KBUILD
...
If everything went well we should see the following message in the kernel log:
```
[  ] Hello from Ada 42
[  ] Goodbye from Ada
```
